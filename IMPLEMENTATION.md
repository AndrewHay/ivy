...
| SD-CONV-6 | §1 Luft "upward bias" | +Y up | **Corrected 2026-08-23 (SD ruling ivy-fhh).** The earlier label "≈ phototropism, replaced by the light field (§15/§16)" was wrong on both counts: a constant upward bias approximates **negative gravitropism** (orienting against gravity), not phototropism, and it was *not* equivalently replaced — the light gradient's vertical component is structurally zero on any flat or convex wall (`SVF = 0.5` at every height, SD-CONV-4), so dropping it deleted the plant's only vertical signal and the climb became momentum from the seed's initial `up` heading. ivy-fhh **re-introduces** it as an explicit `upward_base · (0,1,0)` term in the growth blend, gated to attached tips so it never fights the AS-6 sag on floating spans (implement: ivy-c7e.3). The `up` literal is `(0,1,0)` (`Conv.UP`). |
