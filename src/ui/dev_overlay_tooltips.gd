...
const _TIPS: Dictionary = {
	...
	"droop_base": "Base leaf droop angle (not wired). Would hang leaves downward; no effect until implemented.",
	"droop_shade_gain": "Extra droop in shade (not wired). Would make shaded leaves hang lower.",
	"leaf_jitter_tilt": "Random tilt variation (not wired). Would break up uniform leaf planes.",
	"leaf_jitter_roll": "Random roll variation (not wired). Would add subtle twist between neighbours.",
	"leaf_jitter_yaw": "Random yaw variation (not wired). Would scatter leaf facing around the stem.",
	...
	"leaf_order_falloff": "Size reduction on higher branch orders (not wired). Would shrink twig leaves.",
