...
| Path | Summary |
|------|---------|
| `src/metrics/coverage.gd` | `CoverageMetric` — surface-bucket occupancy in tower-cylindrical coordinates (SD-METRIC-1/2/3/5/6), opening exclusion, stem-bucket diagnostic, 12-sector asymmetry, sun/shade split by seed azimuth. `setup(spec, params)` requires `IvyParams` to compute `_ref_area = alpha_fill("a")·leaf_width_base²/aspect("a")` (AR-METRIC-1 amendment). `measure()` weights every leaf by `_ref_area · |dot(n_leaf, n_bucket)|` — constant per leaf, independent of variant/tier/s_light. Also returns `_nwall`-suffixed diagnostic keys using radial wall normal to quantify the cant's projection contribution. |
