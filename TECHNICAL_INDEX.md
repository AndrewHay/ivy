...
| `src/sim/leaf_placer.gd` | `LeafPlacer` — internode spacing, phyllotaxy, orientation stack, size model, atlas tier selection, colour, and crowding suppression. Emits fully-resolved leaf instances into `PlantData`. **All stochastic leaf attributes are hash-based (`Hash64.unit_float`, `SD-RNG-6`) — zero per-tip stream draws (INV-7, W-064).** W-060: SD-LEAF-6 tier selection (channels 43/44/45, adjacency via `Tip.last_leaf_id`/`prev_leaf_id`); SD-LEAF-7 sun/shade tint `lerp(shade_tint,sun_tint,f_L)`; SD-LEAF-5 `s_light` only; SD-LEAF-4 rule 5 phototropic cant (`n_leaf`). Crowding deposit decoupled from presentation vars (canonical leaf-"a" area at n_wall-offset position). `l_dir` threaded from `GrowthStep.step_tip`. W-075: passes `f_l` as `leaf_light` (no draw). |
...
| `test/test_determinism.gd` | W-042, AS-4 simulation half: two independently built runs from seed 12345 over 120 ticks (5 game-days) produce identical segment count, leaf count, tip count, root-tip **growth-step** stream draw count (branch + correlated-random only — `LeafPlacer` is hash-based per `SD-RNG-6` and advances no stream), and float64 total stem length. Exact equality only — per the AS-4 standing rule a tolerance would make this a Lyapunov divergence meter (SD-OPEN-12 confines tolerance to the image half). Verified by mutation: swapping one `tip.stream.randf()` in `growth_step.gd` for a global `randf()` (the W-033 defect) fails all five assertions, 18 vs 28 segments. |
| `test/test_params_conformance.gd` | INV-6 dead-knob guard (W-042). Every exported parameter must be read by a consumer or derived by an `IvyParams` helper; the declared-but-unread set must equal the documented `UNIMPLEMENTED` list *exactly*, so it fails both when a new dead knob appears and when a listed one is implemented without being delisted. 3 tests, verified by reverting the W-039 fix. Found W-086 (eleven unimplemented knobs) on first run. AR-PARAM-3's §30 literal scan is deliberately absent — see W-085 for why it is not implementable as specified. |
...
| `tools/bake_mesh_sdf.py` | Offline narrow-band SDF baker (SD-MESH-9/10). Reads a sim GLB, writes the `IVYSDF1` binary (`assets/structures/*_sim.sdf`) verified by `src/world/mesh_sdf.gd`. Defaults `--cell 0.05 --band 0.55` (matches the committed SDFs). |
| `tools/measure_structure_gates.py` | Offline Director gate measurements for the structure assets (SD-OPEN-22/23/27). Gate 0 corner-storey continuity, Gate 1 wall thickness (≥0.30 m), Gate 2 interior volume (clear span ≥2 m), Gate 3 hero/proxy \|Φ\| over ivy-reachable hero vertices (≤0.05 m). Run: `python3 tools/measure_structure_gates.py [square tower]`. |
| `tools/diagnose_hero_deviation.py` | Offline hero-vertex deviation diagnostics and `classify_node` node-category mapping shared by `measure_structure_gates.py` (drives the ivy-reachable subset and Gate 0 corner classification). |
