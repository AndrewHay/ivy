...
| `test/test_determinism.gd` | **[planned]** Two identical runs produce identical tip count, bit-identical stem length, identical leaf count and run hash. Exact equality only — per the AS-4 standing rule a tolerance here would make it a Lyapunov divergence meter (SD-OPEN-12 confines the tolerance to the image half). |
